...
import { Edit, LoaderCircle, User, X, Trash2 } from "lucide-react";
...
const ProfileForm = () => {
  const [openModal, setOpenModal] = useState<null | "password" | "username" | "delete">(null);
  ...
  return (
    <div className="flex gap-3 flex-wrap mx-3 sm:mx-0">
      ...
      {/* Password Modal */}
      <Dialog open={openModal === "password"} onOpenChange={() => setOpenModal(null)}>
        <DialogContent className="sm:max-w-lg">
          ...
          <form
            ...
            onSubmit={async (e) => {
              ...
              await handlePasswordSubmit(formData);
            }}
          >
            <div className="flex flex-col gap-3">
              <Label>پسورد فعلی</Label>
              <Input type="password" name="oldpassword" pattern="^.{8,}$" />
              <p className="text-xs text-gray-500 mt-1">پسورد باید حداقل 8 کاراکتر باشد</p>
            </div>
            <div className="flex flex-col gap-3">
            ...
            <DialogFooter>
              <Button type="submit" className="w-full">
                {isLoading ? <LoaderCircle className="animate-spin w-5 h-5 mx-auto" /> : "ذخیره"}
              </Button>
            </DialogFooter>
          </form>
        </DialogContent>
      ...
      {/* Name Modal */}
      <Dialog open={openModal === "username"} onOpenChange={() => setOpenModal(null)}>
        <DialogContent className="sm:max-w-lg">
          <DialogHeader>
          ...
          <form
            ...
            onSubmit={async (e) => {
              e.preventDefault();
              const formData = new FormData(e.currentTarget);
              await handleNameSubmit(formData);
            }}
          >
            <div className="flex flex-col gap-3">
      ...
      <Dialog open={openModal === "delete"} onOpenChange={() => setOpenModal(null)}>
        <DialogContent className="sm:max-w-md">
          ...
          <DialogFooter className="flex gap-2">
            <Button className="flex-1" onClick={() => setOpenModal(null)}>انصراف</Button>
            <Button className="flex-1" variant="destructive" onClick={handleDeleteAccount}>
              {isLoading ? <LoaderCircle className="animate-spin w-5 h-5 mx-auto" /> : "حذف"}
            </Button>
          </DialogFooter>
        </DialogContent>
      </Dialog>
